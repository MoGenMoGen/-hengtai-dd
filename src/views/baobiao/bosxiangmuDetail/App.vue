  ...
  <div class="content">
    ...
    <div class="top">
      ...
      <div class="bodyContent">
        <div class="workHours" v-if="list.length > 0">
          总计工时：{{ total ? list[0].count : "0.00" }}H
        </div>
        <div class="searchBox">
          <div class="boxOne">
          ...
          >
            ...
            <p v-if="dateTime" style="color: #000; margin-left: 0.14rem">
              {{ dateTime }}
            </p>
            <img
              :src="close"
              style="position: absolute; right: 0.15rem"
...
export default {
  ...
  mounted() {
    ...
    if (this.userInfo) {
      ...
      if (this.userInfo.detail.chargeDepts) {
        this.deptIds =
          this.deptIds + "," + this.userInfo.detail.chargeDepts.join(",");
      }
    }
    if (
      this.userInfo &&
      this.userInfo.detail.isCharge == 1 &&
      this.userInfo.detail.chargeDepts
    )
      this.currentRole = 1;
    if (this.userInfo && this.userInfo.role_name.indexOf("boss") != 1)
      this.currentRole = 2;
  ...
  methods: {
        ...
        this.api
          ...
          .then((res) => {
            ...
            res.records.forEach((item) => {
              item.planHours = item.planHours
                ? parseFloat(item.planHours).toFixed(2)
                : "0.00";
              item.workHours = item.workHours
                ? parseFloat(item.workHours).toFixed(2)
                : "0.00";
            });
    ...
    pepDetial(item) {
      this.until.href(
        `/views/baobiao/bosrenyuanDetail.html?userNm=${item.userName}&deptNm=${item.deptName}&deptId=${item.deptId}&userId=${item.userId}&dateTime=${item.workDate}`
      );
    },
  },
};
