<template>
	<div class="content">
		...
		<van-popup v-model="showPicker1" round position="bottom">
			<van-datetime-picker :filter="filter" ref="tmBox" v-model="currentTime1" type="time" title="选择开始时间"
				:min-hour="startTm" :max-hour="endTm" :min-minute="startMi" :max-minute="endMi"
				@cancel="showPicker1 = false" @confirm="onConfirm1" @change='changeTm' />
		</van-popup>
		<van-popup v-model="showPicker2" round position="bottom">
			<van-datetime-picker :filter="filter" v-model="currentTime2" type="time" title="选择结束时间" :min-hour="startTm"
				:max-hour="endTm" :min-minute="startMi" :max-minute="endMi" @cancel="showPicker2 = false"
	...
	export default {
						...
						for (let j = i + 1; j < val.length; j++) {
							...
							if (val[i].id && val[i].id2) {
								if (val[j].id && val[i].id == val[j].id && !val[j].id2) {
									...
									this.index2 = j
									return
								} else if (`${val[i].id}+${val[i].id2}` == `${val[j].id}+${val[j].id2}`) {
									this.index1 = i
									this.index2 = j
									return
								} else {
		...
		methods: {
			...
			checkSelcetTwo(index, index1) {
				this.$set(this.pickService[index].checkList[index1], 'flag', !this.pickService[index].checkList[index1]
					.flag)
