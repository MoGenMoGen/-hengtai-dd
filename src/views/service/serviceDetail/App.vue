...
<template>
    <div id="container">
        ...
        <classify-index @choose="typeConfirm" @close="cancel" v-if="typeSHow"></classify-index>
        <div class="main">
            ...
            <div class="situation">
                <p><i>*</i><span>当前公里数（万）：</span><input v-model="form.curMileage" type="number"/></p>
            </div>
            <div class="situation">
                <div class="title"><span></span>车辆情况</div>
                <div class="content">
                    <p><span>车辆外观：</span><input type="text"  placeholder="请输入车辆外观描述" v-model="form.vehicleOutRemark"/></p>
                    <div class="upImg">
                        <span>上传图片：</span>
                        <div class="imgList">
                            ...
                            <p><input ref="upload" type="file" name="file"   multiple="multiple" @change="upImg($event,'out')"/>
                                +</p>
                        </div>
                    </div>
                </div>
                <div class="content">
                    <p><span>车辆内部：</span><input type="text" placeholder="请输入车辆内部描述" v-model="form.vehicleInRemark"/></p>
                    <div class="upImg">
                        <span>上传图片：</span>
                        <div class="imgList">
                            <p class="img" v-for="(item,index) in inImg" v-if="inImg.length>0">
                                <span class="iconfont" @click="toDele(index,'in')">&#xe673;</span>
                                <img :src="item"  @click="bigImg(item)"/>
                            </p>
                            <p><input ref="upload" type="file" name="file"  multiple="multiple"  @change="upImg($event,'in')"/>+</p>
                        </div>
                    </div>
                </div>
                <div class="content">
                    <p><span>备注信息：</span><textarea v-model="form.rmks"></textarea></p>
                    <p class="red">*贵重物品已提醒客户带走</p>
                </div>
            </div>
            <div class="situation situation2" v-for="(item,index) in prepareList">
                <span class="iconfont" @click="delePrepare(index)" v-if="index>0">&#xe673;</span>
                <div class="title"><span></span>整备项</div>
                <div class="content">
                    <p>
                        <i>*</i><span>整备分类：</span>
                        <button @click="toChoose(index)">{{item.prepareName ? item.prepareName : '请选择整备分类'}}</button>
                        <span class="iconfont">&#xe638;</span>
                    </p>
                    <p><span>整备明细：</span><input type="text" v-model="item.rmks"/></p>
                   <p><span>修理厂：</span><input type="text" v-model="item.garage"/></p>
                    <p><i>*</i><span>金额（成本）：</span><input type="number" v-model="item.cost"/></p>
                    <p><i>*</i><span>金额（销售）：</span><input type="number" v-model="item.price"/></p>
                </div>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            typeConfirm(data){
                ...
                this.prepareList[this.currentIndex].prepareName = this.prepareCurrent.name
            ...
            async upImg(e,type){
                ...
                if(blob.length>10 || (type=='out' && blob.length+this.outImg.length>10) || (type=='in' && blob.length+this.inImg.length>10)){
                    Toast('最多只能上传10张图片')
                    return
                }
            ...
            toSave(type){
                ...
                if(type==='submit'){
                    ...
                    let index = this.prepareList.findIndex(item=>{
                        return (item.cost==='' || item.price === '' || item.prepareId === '' || item.prepareName==='')
                    })
                    if(index>-1){
